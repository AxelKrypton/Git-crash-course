...
\begin{frame}[c,fragile]
    ...
    \begin{onlyenv}<5>
        \begin{lstlisting}[style=MyBash]
            ...
            pick f8b0c25 Improve flaps of wings                  #3
            |*reword*| e499d89 Deploy engine turbo                   #4
        \end{lstlisting}
    \end{onlyenv}
    \begin{onlyenv}<4-5>
    ...
    \FrameRemark<4-5>{Depending on your Git version or \URL*{https://stackoverflow.com/a/32038985}{your Git configuration}, this might appear slightly different.}
\end{frame}
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
\newsavebox{\savedTikZ}
...
\AlertFrame{%
    \begin{tabular}{cc}
        ...
        \includegraphics[height=3cm, clip, trim=0 5mm 0 3mm]{MatrixRebase}\\[6mm]
    \end{tabular}
}
%===============================================================%
...
\section{Git-flow}
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
\begin{frame}<beamer>{\\ \colorbox{PP}{\makebox[1em]{\textcolor{BGLIGHT}{5\vphantom{$A^{\int}_x$}}}}\;Git-flow}
    \vspace{3mm}
...
\AlertFrame{%
    \begin{tabular}{c}
        ...
        But, wait, everything by hand?!\\[12mm]
    \end{tabular}
}
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
...
\begin{frame}{No, it is not!}
    ...
    \begin{onlyenv}<1>
        \begin{center}
            ...
            \includegraphics[width=\textwidth]{Trunk}\\[2mm]
...
\end{frame}
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
\begin{frame}[fragile]{Git evolves steadily}{Just to mention few news}
    \vspace{-2mm}
    \begin{enumerate}
        \item In version 2.54 Git introduced\\
              \begin{lstlisting}[style=MyBash, aboveskip=3mm, xrightmargin=15mm,
                                 moredelim={[is][\color{basic-color}]{|+}{+|}}]
                  # Like 'reword' action of interactive rebase
                  |+git history reword <commit>+|
                  # Handier version than 'edit' in interactive rebase
                  |+git history split <commit>+|
              \end{lstlisting}
        \item In the same version Git introduced config-based hooks\\
              \begin{lstlisting}[style=MyBash, aboveskip=3mm, xrightmargin=15mm,
                                 moredelim={[is][\color{basic-color}]{|+}{+|}}]
                  [hook "linter"]
                      event = pre-commit
                      |+command = ~/path/to/your/linter [options]+|
              \end{lstlisting}
        \item Git version 3.0 is planned for later this year\\
              \then \PP{new repositories will use \texttt{sha256} algorithm by default!}
    \end{enumerate}
\end{frame}
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~%
\tikzset{
...
\begin{frame}{Thanks for listening and remember}
    \begin{varblock}{alert}[\textwidth]{In case of fire:}
        \begin{enumerate}
            \item \bash|git add -A|
            \item \bash|git commit -m 'fvfsdavg'|
            \item \bash|git push|
